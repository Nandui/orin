...
// X-style right column: search, trending stories, topics, releases. Sits in the
// flex flow and follows page scroll (sticky self-start) — no second scrollbar.
export function RightRail({ trending }: { trending: Story[] }) {
  return (
    <aside className="sticky top-0 hidden h-fit w-[330px] shrink-0 self-start px-5 py-4 xl:block">
      ...
      <section className="mb-4 overflow-hidden rounded-2xl bg-[#16181c]">
        <h2 className="flex items-center gap-2 px-4 pt-3 text-xl font-extrabold text-white">
        ...
        <div className="mt-1">
          ...
          {trending.length ? (
            <Link
              href="/?sort=trending"
              className="block px-4 py-3 text-sm text-[var(--accent)] transition-colors hover:bg-white/[0.03]"
            >
              Show more
            </Link>
          ) : null}
      ...
      <section className="mb-4 overflow-hidden rounded-2xl bg-[#16181c] p-4">
        <h2 className="mb-2 text-xl font-extrabold text-white">Topics</h2>
      ...
      <section className="mb-4 overflow-hidden rounded-2xl bg-[#16181c]">
        <h2 className="px-4 pt-3 text-xl font-extrabold text-white">Upcoming</h2>
        <ul className="mt-1">
          {UPCOMING.map((r) => (
            <li key={r.title}>
              <Link
                href="/releases"
                className="flex items-center justify-between gap-2 px-4 py-3 transition-colors hover:bg-white/[0.03]"
              >
      ...
      <AdSlot slot="sidebar" className="overflow-hidden" />
    </aside>
  );
}
